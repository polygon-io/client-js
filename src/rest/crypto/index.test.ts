...
describe('[REST] Crypto', () => {
	...
	it('sma call /v1/indicators/sma/{cryptoTicker}', async () => {
		...
		requestStub
			.getCalls()[0]
			.args[0].should.eql('/v1/indicators/sma/X:BTC-USD');
	});
	...
	it('ema call /v1/indicators/ema/{cryptoTicker}', async () => {
		...
		requestStub
			.getCalls()[0]
			.args[0].should.eql('/v1/indicators/ema/X:BTC-USD');
	});
	...
	it('macd call /v1/indicators/macd/{cryptoTicker}', async () => {
		...
		requestStub
			.getCalls()[0]
			.args[0].should.eql('/v1/indicators/macd/X:BTC-USD');
	});
	...
	it('rsi call /v1/indicators/rsi/{cryptoTicker}', async () => {
		...
		requestStub
			.getCalls()[0]
			.args[0].should.eql('/v1/indicators/rsi/X:BTC-USD');
	});
});
